...
from report_utils import (
    ...
    build_modalities_html,  # for multimodal runs (not used in Features tab anymore)
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Binary decision threshold in [0,1]. If omitted, default 0.5 (argmax) is used.",
    )
    ...
    # ===== Build extra rows for Model Summary & Config → Run Configuration =====
    label_col = args.label_column
    image_col = args.image_column

    # Count tabular columns (exclude label and image columns if present)
    tabular_cols = [c for c in df_train_full.columns if c not in {label_col, image_col}]
    tabular_count = len(tabular_cols)

    # Modalities & Inputs summary text (moved from Features tab)
    ...
    # Presets (we don't set presets explicitly; AutoGluon defaults to 'medium')
    presets_used = "medium"

    # Decision threshold calibration description based on validation size
    calib_text = (
        "disabled (due to <10,000 validation rows (to avoid overfitting))"
        if len(df_val) < 10_000
        else "enabled"
    )

    # Decision threshold value formatting
    threshold_val = "None"
    if args.threshold is not None:
        try:
            threshold_val = f"{float(args.threshold):.3f}"
        except Exception:
            threshold_val = str(args.threshold)
    ...
    extra_run_rows = [
        ...
        ("Image column", image_col if image_col else "—"),
        ("Tabular columns", str(tabular_count)),
        ("AutoGluon", f"'{presets_used}' (fast prototyping)"),
    ...
    # Class Balance (Train Full) moved to Model Summary & Config tab
    class_balance_block_html = build_class_balance_html(df_train_full, label_col)

    # ===== Summary tab (Model Summary & Config) =====
    # NOTE: build_summary_html must accept extra_run_rows and class_balance_html for this to work.
    # FIRST TABLE in Model Summary & Config: all splits (Train/Val/Test) metrics
    summary_perf_table_html = build_model_performance_summary_table(
        ...
        include_test=True,   # ← include Test column here
        title=None,          # ← no inner black <h3>
        show_title=False,
    )
    ...
    train_tab_perf_html = build_model_performance_summary_table(
        ...
        title=None,          # no inner black subtitle
        show_title=False,
    )
    ...
    if is_multimodal:
        # For multimodal, we do not have permutation importance here.
        feature_html = (
        ...
        # Tabular: keep permutation importance; we are NOT adding modalities or class balance here anymore.
        feature_html = build_feature_html(predictor, df_test, args.label_column, tmpdir, args.random_seed)

    # ===== Notices & transparency (minus the items we moved) =====
    notices: List[str] = []
    ...
    if args.threshold is not None and kind == "binary":
        notices.append(f"using decision threshold = {float(args.threshold):.3f} for binary label generation.")
    ...
    # We previously added a "Modalities & Inputs" block and "Class Balance (Train Full)" to the Features tab.
    # Both have been MOVED to the Model Summary & Config tab. So do NOT include them here.
    inputs_html = ""  # build_modalities_html(...) moved out of Features tab
    ...
    # Only include transparency items that still belong in the Features tab
    transparency_blocks = "\n".join(
        [
            leaderboard_html,
            inputs_html,  # empty (kept for layout consistency)
